...
export const generateDoseTableNotis = (data, furnaceNumber, currentTime = new Date().toLocaleString()) => {
  if (!data) return 'Нет данных для отображения.';
  ...
  // Параметры нотис
  const doses = [
    formatDose('Г/мин', `Дозатор ВР${furnaceNumber} Г/мин`, 'г/мин'),
    formatDose('Кг/час', `Дозатор ВР${furnaceNumber} Кг/час`, 'кг/час'),
    formatDose('Общий вес в граммах', `Дозатор ВР${furnaceNumber} Общий вес в граммах`, 'г'),
    formatDose('Количество штук', `Дозатор ВР${furnaceNumber} Количество штук`, 'шт.'),
    formatDose('Общий вес в тоннах', `Дозатор ВР${furnaceNumber} Общий вес в тоннах`, 'тонн'),
  ];
